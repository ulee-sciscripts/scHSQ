...
This data contains single-cell RNA-Seq data from three species: \textit{D. melanogaster}, \textit{D. yakuba}, and \textit{D. ananassae}. The data is wrapped in a \Rpackage{monocle3} package with species information encoded in the ``$spec$" field and cell type information encoded in the ``$known_type$" field, with reference species labeled as ``$mel$". Each species' data set was individually imported using \Rpackage{monocle3}'s $new_cell_data_set$ function, reduced to retain only one-to-one orthologs, then combined using \Rpackage{monocle3}'s $combine_cds$ function. Subsequently, \Rpackage{monocle3}'s $preprocess_cds$, $reduce_dimension$, and $cluster_cells$ functions were applied. 

The resulting object is called ``testis_3sp". We thus use the known labels to generate a new \Rpackage{scHSQ} object as following:
...
After these commands complete, a cross-species UMAP projection should be stored in the ``reducedCDS" slot of the \Rpackage{scHSQ} object. We can extract the list of marker genes utilized for this UMAP projection. Additionally, we can produce plots on the cross-species UMAP projection. In this case, the ``ctplots" object is a list of UMAP projections containing: 1) known cell type labels for the reference species, 2) new clusters for all cells across all species, and 3) clusters for cells in each respective species. Note that arguments for \Rpackage{monocle3}'s ``plot_cells" can be passed to \Rpackage{scHSQ}'s ``plotCTAssign" function and will automatically be applied to all elements of ``ctplots". All following figures were generated using ``width=1500, height=1500, res=300, pointsize=5".
...
\caption{Single UMAP of original cell type assignments for reference species. This is found in ctplots$\[\[1\]\]$.}
\end{figure}

\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{scHSQ_UMAP_all}
\caption{Single UMAP of new cell type assignments for all species. This is found in ctplots$\[\[2\]\]$.}
\end{figure}
...
\caption{Single UMAP of re-assigned cell types for reference species. This is found in ctplots_reassign$\[\[2\]\]$.}
\end{figure}
...
Finally, we may be interested in seeing a single gene's expression on the new UMAP. To generate this figure, we use the example of \textit{Rbp4}, a gene that did not pass the HSQ criteria. We utilize the ``plotGeneExprs" function to generate this figure. As before, we are also able to forward arguments for \Rpackage{monocle3}'s ``plot_cells" function. We may also generate this plot for a single species' cells by using the ``species=" argument.
